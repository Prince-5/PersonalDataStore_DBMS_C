...
struct BST_Node* minKeyNode(struct BST_Node* node) 
{ 
...
struct BST_Node* delete_node_by_key(struct BST_Node* root, int key) 
{  
    if (root == NULL) 
	return root; 
   
    if (key < root->key) 
        root->left_child = delete_node_by_key(root->left_child, key); 
  
    else if (key > root->key) 
        root->right_child = delete_node_by_key(root->right_child, key); 
    ...
    { 
        ...
        struct BST_Node* temp = minKeyNode(root->right_child); 
   
        root->key = temp->key;
		root->data = temp->data; 
   
        root->right_child = delete_node_by_key(root->right_child, temp->key); 
    } 
    return root; 
}
...
{
	struct BST_Node* newrootptr = delete_node_by_key(*root,key);
	*root=newrootptr;
	return BST_SUCCESS;
}
